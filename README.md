...
OpenAIとOpenGLを用いた3D手旗信号シミュレーションツールです。
コマンドプロンプトから入力された文字列を反映し、3Dキャラクターが手旗信号のアニメーションを行います。

https://github.com/user-attachments/assets/72a91cbe-3164-458a-8ebe-25470d8d44c7
...
本システムは、 **「高度な言語処理を行うPythonクライアント」** と **「リアルタイム3D描画を行うC言語(OpenGL)サーバー」** をTCPソケット通信で連携させたハイブリッド構成となっています。
...
* **通信フロー:**
    ...
    2.  **C言語側 (Server):** パケットを受信・解析し、OpenGLウィンドウ上でアニメーションを実行します。
    3.  **同期制御:** アニメーション終了時には、C言語側からPython側へ完了通知を送ることで、音声対話と身体動作のタイミング同期を行っています。
